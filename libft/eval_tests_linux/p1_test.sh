for f in eval_tests_linux/p1_tests/*.test
do
	mv "$f" "${f%.test}"
done
...
gcc -Wall -Wextra -Werror -o test_part1 eval_tests_linux/p1_tests/*.c -L. -lft
./test_part1
rm norm_check norm_errors make_check test_part1 libft.a eval_tests_linux/libft.h
