for f in eval_tests_linux/p2_tests/*.test
do
	mv "$f" "${f%.test}"
done
echo "PART 2 norm check"
~/.norminette/norminette.rb ft_itoa.c ft_memalloc.c ft_memdel.c \
...
gcc -Wall -Wextra -Werror -o test_part2 eval_tests_linux/p2_tests/[^p]*.c -L. -lft
./test_part2
./eval_tests/p2_tests/put_tests.sh
rm norm_check norm_errors make_check test_part2 libft.a eval_tests_linux/libft.h
